...
def get_all_qa_from_md_files(kb_name):
    ...
    # ======================
    # 核心：Python 后端严格升序排序（中英文都按字典序）
    # ======================
    qa_list.sort(key=lambda x: x["question"].lower())
...
def build_index_incremental():
    ...
    for fname in to_update:
        ...
        try:
            ...
            question = lines[0].strip()
            answer = "\n".join(lines[1:]).strip()
            vec = simple_hash_vector(question)
...
def search():
    ...
    for i in range(len(res["ids"][0])):
        result.append({
            ...
            "answer": res["metadatas"][0][i]["answer"],  # 只返回原文，不转HTML
